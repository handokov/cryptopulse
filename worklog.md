...
Task ID: 31
Agent: Super Z (main agent)
Task: Bot v2 Phase 2 — candlestick chart + trigger lines, timeframe per mode, draggable entry line (AND-scored), pending-entry state, Stop & Sell semantics. User command: "oke, gas fase 2".

Work Log:
- Confirmed Phase 1 already live (Task 30) → Phase 2 scope executed from the frozen design notes (25/26/26-b/26-c/26-d): chart+lines, TF per mode, draggable line with TP/SL auto-follow, stop&sell + manual close.
- bitget-trade.ts: TF_GRANULARITY map + barsPerYearFor() + fetchCandles() (full OHLC ascending, last row = forming bar — same data the engine signal sees). CRITICAL FINDING: Bitget granularity is case-sensitive lowercase — "4h"/"1h"/"1day"/"15min" work; "4H"/"1H"/"1D" return code 400171 (verified live; the legacy "4h" const now explains itself).
- strategy.ts refactor: pure TF tables moved to timeframes.ts (TF_OPTIONS: MODERATE 1H/4H/1D, AGGRESSIVE 15M/30M/1H; TF_DEFAULT 4H/1H; TF_COOLDOWN_MIN 5/10/20/45/240 — 4H keeps the historical 45m) and re-exported so client components never pull node:crypto transitively. computeBotSignal(closes, barsPerYear?) is now TF-aware.
- engine.ts: tf = valid(cfg.timeframe) ?? "4H" (zero regression); bpy drives volAnn + VOL bands; cooldownMinFor(mode, tf); touchConfig() stamps lastTickAt + lastPrice on every completed tick; NEW entry-line gate (3b) — BUY requires score ≥ threshold AND entryLineTouched(line, lastPrice, price): any-touch = ±0.1% epsilon OR crossing between prev and current tick price (gap-safe); entry reason gains "[line … touched]"; exported manualClosePositions() for Stop&Jual (paper → ticker fill; live → signed market SELL + fill refine, same audit trail).
- Schema + migrate: BotConfig.timeframe TEXT DEFAULT '4H', entryLine REAL, lastPrice REAL — three plain ALTERs in ensureBotColumns (no rebuild; auto-applies to Turso on first production hit).
- API: PUT validates timeframe (mode-gated, invalid/absent → mode default) + entryLine (null clears, else positive price); GET summary adds cooldownMin/timeframe/tfOptions; bots[] adds timeframe/entryLine. NEW GET /api/bot/candles (auth, 45s cache): OHLC + live ticker + signal{score,entryScore} + effective bands{tpPct,slPct} mirroring tickOne exactly. NEW POST /api/bot/close {configId}: owner-checked, closes every OPEN position regardless of enabled state, 409 no_position guard.
- bot-chart.tsx (new, lightweight-charts v5.2.1): dark-themed candles + 4 price lines (ENTRY amber dashed draggable; TP emerald / SL red dotted — REAL position levels when open, else preview anchored to the line with live re-anchoring while dragging; BUY muted); drag via pointer capture with ±14px hit test, pan/zoom suspended during drag, commit-on-release; legend = score-vs-threshold badge, line-state chip (at-line / waiting+distance%), TP·SL values, live price; arm-at-price + clear buttons; 60s refetch + refreshKey bump after ticks/stops.
- bot-section.tsx: chart card mounted (non-draft); TF selector (mode-gated, mode switch keeps TF when still allowed else resets to default); enabled-switch intercept when a position is open → Stop dialog ("Stop saja" = disable+save; "Stop & Jual" = close positions then disable); per-row Close position button + confirm; entry-line drag-save with optimistic update + rollback on error.
- i18n: 28 new bot.* keys × 6 languages via scripts/inject-i18n-v2phase2.mjs — parity 523/523 ALL CLEAN.
- Verification: ESLint 0 problems; backend E2E 23/23 PASS (scripts/verify-v2phase2.ts: register→create→candles(4H/1H)→TF coercion(MODERATE+15M→4H, AGGRESSIVE+1D→1H)→cooldown(45/240/20)→entryLine validation→tick stamps lastPrice→close 409 no_position→anon 401); pure-logic unit 20/20 PASS (epsilon/crossing/gap touches, cooldown table, bpy, volAnn sqrt scaling, score TF-invariant); dev-server restart needed after db:push (stale Prisma client — same as Task 30); Agent Browser E2E: anonymous lock ✓, register ✓, bot create ✓, chart renders (4H→1D switch refetch) ✓, score badge ✓, arm line @77750 ✓, DRAG → 81824.07 persisted to DB ✓, waiting chip "(jarak 4.89%)" ✓, seeded position → stop dialog ✓ → Stop & Sell closed at ticker 77850 (+$0.0552, "manual close (stop & sell)" in log, bot disabled) ✓, i18n id strings ✓, 0 console errors ✓, mobile 390px ✓. Initial "double SL label" screenshot was a dev Fast-Refresh artifact — clean after reload.
